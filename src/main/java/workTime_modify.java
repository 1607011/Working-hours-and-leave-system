 ...
 */
public class workTime_modify extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public workTime_modify() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		for (int i = 0; i < (endDay - startDay); i++) {
			if (startDay + i < 10)
				DATE = startDate.substring(0, 8) + "0" + (startDay + i);
			else
				DATE = startDate.substring(0, 8) + (startDay + i);
			System.out.println(DATE);
			if (status == 1 && db.getLeaveStatus(id, DATE) == 1) {
				out.write("10");
				return;
			}
		}
		...
		if (!db.canAdd(task_name, id)) {
			db.deleteTask(task_name, id);
		}
		// 利用JDBC執行SQL指令新增工時紀錄
